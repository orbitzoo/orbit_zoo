...
# Orekit runs on a JVM. Prefer a system Java (via JAVA_HOME); otherwise fall back
# to the pip-installed JDK from `jdk4py` so the project works from pip alone.
if not os.environ.get('JAVA_HOME'):
    try:
        from jdk4py import JAVA_HOME as _JDK4PY_HOME
        os.environ['JAVA_HOME'] = str(_JDK4PY_HOME)
    except ImportError:
        pass

orekit.initVM()

# NOTE: import pyhelpers only AFTER initVM() — unlike the classic `orekit`,
# orekit_jpype.pyhelpers touches Java (`from java.io import File`) at import time,
# which requires the JVM to already be running.
from orekit_jpype.pyhelpers import setup_orekit_curdir

# Orekit data ships bundled inside the package (as a zip); resolve it relative to
# this file so it works no matter the current working directory or install path.
_OREKIT_DATA = Path(__file__).resolve().parent / 'dynamics' / 'orekit' / 'orekit-data.zip'
setup_orekit_curdir(str(_OREKIT_DATA))

from orbitzoo.dynamics.tensorgator.main import TensorgatorDynamics
from orbitzoo.dynamics.orekit.main import OrekitDynamics
from orbitzoo.interface.main import Interface
from orbitzoo.rl_algorithms.ppo import PPO
from orbitzoo.rl_algorithms.dqn import DQN
from orbitzoo.rl_algorithms.ddpg import DDPG
from orbitzoo.rl_algorithms.td3 import TD3
...
class OrbitZoo(ParallelEnv):
    ...
    def train(
        ...
        **Make sure you have implemented *self.rewards()* and *self.observations()* before using this function.**
        ...
        # ensure the destination for saved models exists
        if is_training and not is_optuna and save_path:
